<script lang="ts">
  ...
  // Use the helper function in the filter
  $: filteredResults = !resultsFilterQuery 
    ? results 
    : results.filter(result => {
        const name = extractName(result);
        return name.toLowerCase().includes(resultsFilterQuery.toLowerCase());
      });
  ...
  async function handleSearch() {
    ...
    // Reset filter when performing a new search
    resultsFilterQuery = '';
  ...
  async function handleAudioUpload(event: Event) {
    ...
    // Reset filter when uploading new audio
    resultsFilterQuery = '';
  ...
  function handleDrop(event: DragEvent) {
    ...
    // Reset filter when dropping a new file
    resultsFilterQuery = '';
...
<div class="container">
        ...
        <div class="results-info">
          ...
          <div class="results-count">
            {filteredResults.length === results.length 
              ? `${results.length} samples found` 
              : `${filteredResults.length} of ${results.length} samples found`}
          </div>
        </div>
  ...
  <!-- Add a spacer at the bottom to prevent content from being cut off -->
...
/* Add this to make the SVG icon white */
  ...
  .reset-filter {
    background: rgba(76, 107, 224, 0.2);
    border: 1px solid #4c6be0;
    color: #4c6be0;
    padding: 0.5rem 1rem;
    border-radius: 4px;
    cursor: pointer;
  ...
  .reset-filter:hover {
    background: rgba(76, 107, 224, 0.3);
  }
